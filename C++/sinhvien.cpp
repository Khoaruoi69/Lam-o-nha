...
void TimKiemTheoTen(SV a[], int n, int id);
void tinhDTB(SV&sv);
void xeploai(SV&sv);
...
void nhapSV(SV a[], int n,int id){
	printLine(40);
	printf("\n Nhap sinh vien thu %d: ", n+1);
	nhapThongTinSV(a[n],id);
	printLine(40);
}
void capnhapThongTinSV(SV&sv){
	cout<<"\n Nhap ten"; fflush(stdin); gets(sv.ten);
	cout<<" Nhap gioi tinh: "; gets(sv.gioitinh);
	cout<<"Nhap tuoi: "; cin>>sv.tuoi;
	cout<<"Nhap diem toan: "; cin>>sv.diemToan;
...
void capNhatSV(SV a[], int n, int id){
	int found = 0;
	for(int i=0;i<n;i++){
		if(a[i].id==id){
			found = 1;
			printLine(40);
			cout<<"\n Cap nhat thong tin sonh vien co ID="<<id;
...
void tinhDTB(SV &sv) {
    sv.diemTB = (sv.diemToan + sv.diemLy + sv.diemTin) / 3;    
}
void xeploai(SV &sv) {
...
void showStudent(SV a[], int n) {
  ...
  //  cout <<"\n\STT\tID\tHo va ten\tGioi tinh\tTuoi\tToan\tLy\tHoa\tDiem TB\tHoc luc";
    for(int i = 0; i < n; i++) {
        ...
        printf("\t\t%d", a[i].tuoi);
        printf("\t%.2f\t%.2f\t%.2f", a[i].diemToan, a[i].diemLy, a[i].diemTin);
...
int docFile(SV a[], char fileName[]) {
    ...
    while (fscanf(fp, "%5d%30s%5s%5d%10f%10f%10f%10f%10s\n", &a[i].id, &a[i].ten, 
            &a[i].gioitinh, &a[i].tuoi, &a[i].diemToan, &a[i].diemLy, &a[i].diemTin, 
...
void ghiFile(SV a[], int n, char fileName[]) {
    ...
    for(int i = 0;i < n;i++){
        fprintf(fp, "%5d%30s%5s%5d%10f%10f%10f%10f%10s\n", a[i].id, a[i].ten,a[i].gioitinh, 
            a[i].tuoi, a[i].diemToan, a[i].diemLy, a[i].diemTin, a[i].diemTB, a[i].hocluc);
    }
    fclose (fp);
}
int main() {
